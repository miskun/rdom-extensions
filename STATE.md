...
## 2026-06-02 — time-series gallery: the 7 lens storybook demos ported

Replicated the lens time-series storybook (Smooth, Spiky, Single, Dense, Live, Live Spotty, Empty)
...
**Feature audit vs lens: complete for the demos.** Everything the storybook uses was already
...
- **Time-series "future" styles** (marked `// Future` in `data.rs`, and *also* unbuilt in lens):
...
- **Virtual table rich features** (scoped out of M5): sorting, row/cell selection, column
  ...
  persistence callbacks. The biggest gap vs lens's `VirtualTableComponent`. Candidate next milestone.
- **Other lens-ui components not ported:** `KeyValueList`, `Markdown` (if full lens-ui parity is
